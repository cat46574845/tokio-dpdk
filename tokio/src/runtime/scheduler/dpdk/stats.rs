//! Per-worker statistics for the DPDK scheduler.
//!
//! This is adapted from `multi_thread/stats.rs` with park-related methods removed
//! since DPDK workers never park.
...
pub(crate) struct Stats {
    ...
    /// Instant at which work last resumed.
    ...
    /// Exponentially-weighted moving average of time spent polling a task.
    ///
...
impl Stats {
    ...
    /// Returns the tuned global queue interval based on observed poll times.
    pub(crate) fn tuned_global_queue_interval(&self, config: &Config) -> u32 {
        ...
        // If we are using self-tuning, we don't want to return less than 2 as that
        // would result in the global queue always getting checked first.
        tasks_per_interval.clamp(2, MAX_TASKS_POLLED_PER_GLOBAL_QUEUE_INTERVAL)
    }

    /// Submits the current batch of metrics to the worker metrics.
    pub(crate) fn submit(&mut self, to: &WorkerMetrics) {
        self.batch.submit(to, self.task_poll_time_ewma as u64);
    }

    /// Increments the local schedule count.
    pub(crate) fn inc_local_schedule_count(&mut self) {
        self.batch.inc_local_schedule_count();
    }

    /// Marks the start of processing scheduled tasks.
    ...
    /// Marks the end of processing scheduled tasks and updates EWMA.
    ...
    /// Marks the start of polling a task.
    ...
    /// Marks the end of polling a task.
    pub(crate) fn end_poll(&mut self) {
        self.batch.end_poll();
    }

    /// Increments the overflow count (when local queue is full).
