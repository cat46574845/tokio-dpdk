...
impl Builder {
    ...
    /// with kernel bypass and busy-poll workers. Device configuration is read
    /// from `/etc/dpdk/env.json` (generated by `scripts/dpdk/setup.sh`).
    ///
    ...
    /// use tokio::runtime::Builder;
    ///
    /// // Use all available DPDK devices and cores
    /// let rt = Builder::new_dpdk()
    ///     .enable_all()
    ///     .build()
    ///     .unwrap();
    ///
    /// // Use specific device with 4 workers
    /// let rt = Builder::new_dpdk()
    ...
    pub fn new_dpdk() -> Builder {
        ...
        builder.thread_name = std::sync::Arc::new(|| "dpdk-worker".into());
        builder
    }
    ...
    ///
    /// These arguments are merged with any EAL arguments from env.json.
    /// Builder arguments are applied after env.json arguments (allowing overrides).
    ///
    /// # Example
    ///
    /// ```ignore
    ...
    /// Device configuration (IP, MAC, gateway) is read from `/etc/dpdk/env.json`.
    ///
    /// Use `worker_threads()` for multi-queue configurations where multiple
    /// workers share a single NIC.
    ///
    /// # Example
    ///
    /// ```ignore
    ...
    ///     .worker_threads(4)                      // 4 workers = 4 queues
    ///     .build()
    ///     .unwrap();
    /// ```
    #[cfg(feature = "rt-multi-thread")]
    ...
    /// Sets the DPDK memory pool size (number of mbufs).
    ///
    /// Default: 8192.
    ///
    /// # Example
...
cfg_rt_multi_thread! {
    impl Builder {
        ...
        /// Device configuration is read from env.json. Worker count is controlled
        /// by `worker_threads()`, thread naming by `thread_name()`, etc.
        fn build_dpdk_runtime(&mut self) -> io::Result<Runtime> {
            ...
            let dpdk_builder = self.dpdk_builder.as_ref().ok_or_else(|| {
                io::Error::new(
                    ...
                    "DPDK runtime requires dpdk_builder configuration. \
                     Use Builder::new_dpdk() to create a DPDK runtime.",
                )
            })?;
            ...
            // Use worker_threads for blocking pool estimation
            let worker_threads_estimate = self.worker_threads.unwrap_or_else(num_cpus);

            // Create the blocking pool
            let blocking_pool =
                blocking::create_blocking_pool(self, self.max_blocking_threads + worker_threads_estimate);
            let blocking_spawner = blocking_pool.spawner().clone();
            ...
            // Pass Builder's native thread configuration for worker threads
            let (mut scheduler, mut launch) = Dpdk::new(
                ...
                self.after_start.clone(),
                self.before_stop.clone(),
            )?;
